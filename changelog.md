...
- Disable native window controls when enabling glasstheme (Windows only)
- Added a workaround for .node files being locked by VSCode as "in use", enabling/reloading glasstheme now doesn't require manually deleting the `runtime` folder.
- Code improvements
